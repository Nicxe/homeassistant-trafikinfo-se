...
class TrafikinfoSEConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def __init__(self) -> None:
        ...
        self._traffic_flow_selection: dict[str, Any] = {}
    ...
    def _route_title_for_reconfigure(
        ...
        route: TravelTimeRouteCatalogEntry,
        raw_name: str,
    ) -> str:
        ...
        if (
            current_title
            and raw_name == current_title
            and current_title == previous_route_name
        ):
            return route.name
        return raw_name
    ...
    async def async_step_reauth(self, _entry_data: dict[str, Any]):
        """Start reauthentication after Trafikverket rejects the API key."""
        return await self.async_step_reauth_confirm()

    async def async_step_reauth_confirm(self, user_input: dict[str, Any] | None = None):
        """Validate and store a replacement Trafikverket API key."""
        errors: dict[str, str] = {}
        if user_input is not None:
            api_key = str(user_input.get(CONF_API_KEY, "")).strip()
            if not api_key:
                ...
                try:
                    result = await _async_test_api_key(self.hass, api_key)
                    if not result.ok:
                        raise InvalidAuth(result.error_message or "invalid auth")
                except InvalidAuth as err:
                    _LOGGER.debug("Invalid auth during reauthentication: %s", err)
                    errors["base"] = "invalid_auth"
                except CannotConnect as err:
                    _LOGGER.debug("Cannot connect during reauthentication: %s", err)
                    errors["base"] = "cannot_connect"
                except Exception:
    ...
    async def async_step_entry_kind(self, user_input: dict[str, Any] | None = None):
        ...
        schema = vol.Schema(
            {
                vol.Required(CONF_ENTRY_KIND, default=ENTRY_KIND_INCIDENT): selector(
                    {
                        "select": {
    ...
    async def async_step_traffic_flow_site(
        ...
        if user_input is not None:
            ...
            if site is None:
                ...
                self._traffic_flow_selection = dict(user_input)
                return await self.async_step_traffic_flow_confirm()
        ...
        schema = vol.Schema(
            {
                vol.Required(
                    CONF_TRAFFIC_FLOW_SITE_IDS,
                    default=self._traffic_flow_selection.get(
                        CONF_TRAFFIC_FLOW_SITE_IDS, site_options[0]["value"]
                    ),
                ): selector(
                    {
                        "select": {
                ...
                vol.Optional(
                    CONF_NAME, default=self._traffic_flow_selection.get(CONF_NAME, "")
                ): str,
            }
        )
        return self.async_show_form(
    ...
    async def async_step_traffic_flow_confirm(
        ...
        """Preview the selected site's position without changing an entry."""
        site = self._traffic_flow_site_by_key(
            self._traffic_flow_selection.get(CONF_TRAFFIC_FLOW_SITE_IDS, "")
        )
        if site is None:
        ...
        latitude = f"{site.latitude:.5f}"
        longitude = f"{site.longitude:.5f}"
        prefix = "reconfigure_" if self._reconfigure_entry is not None else ""
        return self.async_show_menu(
            step_id="traffic_flow_confirm",
            menu_options=[f"{prefix}traffic_flow_save", f"{prefix}traffic_flow_site"],
            description_placeholders={
                "latitude": latitude,
                "longitude": longitude,
                "distance": f"{site.distance_km:.1f}",
                "map_url": (
    ...
    async def async_step_traffic_flow_save(
        self, user_input: dict[str, Any] | None = None
    ):
        """Continue setup after the user has confirmed the measurement site."""
        site = self._traffic_flow_site_by_key(
            self._traffic_flow_selection.get(CONF_TRAFFIC_FLOW_SITE_IDS, "")
        )
        if site is None:
            return await self.async_step_traffic_flow_site()
        user_input = self._traffic_flow_selection
        default_name = (
            f"Trafikflöde – {self._traffic_flow_direction_label(site.measurement_side)}"
        )
        ...
        data = {
            ...
            CONF_TRAFFIC_FLOW_SITE_LABEL: self._traffic_flow_site_option_label(site),
    ...
    async def async_step_reconfigure_traffic_flow_save(
        self, user_input: dict[str, Any] | None = None
    ):
        """Apply a replacement site only after the location is confirmed."""
    ...
    async def async_step_reconfigure(self, user_input: dict[str, Any] | None = None):
        ...
        if entry_kind == ENTRY_KIND_TRAVEL_TIME_ROUTE:
            ...
            if (
                current_route_county not in COUNTIES
                and current_route_county != COUNTY_ALL
            ):
        ...
        schema = vol.Schema(
            {
                vol.Required(
                    CONF_TRAFFIC_FLOW_SITE_IDS,
                    default=self._traffic_flow_selection.get(
                        CONF_TRAFFIC_FLOW_SITE_IDS, default_site
                    ),
                ): selector(
                    {
                        "select": {
                ...
                vol.Optional(
                    CONF_NAME,
                    default=self._traffic_flow_selection.get(
                        CONF_NAME, entry.title or "Trafikflöde"
                    ),
                ): str,
            }
        )
    ...
    async def async_step_reconfigure_coordinate(
        ...
        if user_input is not None:
            ...
            new_data.update(
                {
                    CONF_ENTRY_KIND: self._entry_kind,
            ...
            if self._entry_kind == ENTRY_KIND_INCIDENT:
                new_data.update(
                    {
    ...
    def _schema_common_tail(
        ...
        *,
        ...
        if self._entry_kind == ENTRY_KIND_ROAD_CONDITION:
            return {
